<template>
	<view class="z-table">
		<view class="z-table-main" :style="compluteHeight">
			...
			<view class="z-table-container">
				<view class="z-table-pack">
					<view class="z-table-title">
						<view class="z-table-title-item" :class="{ 'z-table-stick-side': stickSide && index == 0 }" :style="{ width: item.width ? item.width : '200rpx' }"
					...
					<view v-if="tableData.length" :class="['table-container-box', {'short-table': !longTable && showBottomSum}]">
						<view class="z-table-container-row" :class="{ 'z-table-has-bottom': showBottomSum }" v-for="(row, iIndex) in tableData"
						 :key="iIndex">
							<view :class="['z-table-container-col', { 'z-table-stick-side': stickSide && jIndex == 0 }]" :style="{ width: col.width ? col.width  : '200rpx' }"
			...
			<view v-if="tableData && tableData.length == 0 && !isLoading" class="table-empty">
				<!-- <image class="empty-img" src="@/static/tool/nodata.png"></image> -->
				<view>~ 暂无数据~ </view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			getRowContent(row, col) {
				...
				// item.memberCount = item.memberCount>0?`${item.memberCount}(赠${item.memberCount})`: item.memberCount;
				// 		item.tradeAmount = toStringHandle(item.tradeAmount);
				// 		item.estimatedIncomes = toStringHandle(item.estimatedIncomes);
				// 		item.platformReward = toStringHandle(item.platformReward);
				// 		item.netIncomes = toStringHandle(item.netIncomes);
				let tempHTML = ''
				if(col.key == 'memberCount'){
                    row[col.key] = row[col.key]>0?`${row[col.key]}(赠${row[col.key]})`: row[col.key];
				}
...
<style lang="scss">
	...
	.z-table {
		...
		.z-table-container-row {
			z-index: 0;
			// border-bottom: solid 2rpx rgb(202, 199, 199);
			box-sizing: border-box;
		}
		.z-table-container-row:nth-child(even) {
